...
def calc_dependent_variables(sigma_v, sigma_veff, f_s, p_a, q_t):
    ...
    for dd in range(0, num_depth):
        ...
        while True:
            ...
            current_n = max(0.5, 0.381 * i_c[dd] + 0.05 * (sigma_veff[dd] / p_a) - 0.15)
            #print(n_val, 0.381 * i_c[dd] + 0.05 * (sigma_veff[dd] / p_a) - 0.15)
            if n_val - current_n < 0.01:
                break
            else:
                n_val = current_n
        #print("exit 2nd loop,",n_val, f'{current_n:.2f}', n_val - current_n)
    return big_q, big_f, i_c

def k_sigma(i_c,big_f):
    """
    Returns k_sigma
    i_c values higher than 2.6 and lower than 1.64 have 1
    """
    cond_1 = (lf.i_c <= 1.64) | (lf.i_c > 2.6)
    cond_2 = ((lf.i_c > 1.64) & (lf.i_c <= 2.36)) & (lf.big_f < 0.5)
    where__ = np.where(cond_2, 1,
                       5.581 * lf.i_c ** 3 - 0.403 * lf.i_c ** 4 - 21.63 * lf.i_c ** 2 + 33.75 * lf.i_c - 17.88)
    np.where(cond_1, 1, where__)
...
plt.plot(lf.i_c,lf.depth, label = 'R&W')
plt.plot(bi2014.i_c,lf.depth, label = 'B&I', color = 'r')
plt.legend()
plt.show()
print(bi2014.i_c)